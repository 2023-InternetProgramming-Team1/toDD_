...
def assignment_creation(request, activity_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            assignment.save()
            return redirect('eclass/lecture_detail', lecture_id=activity.lecture.id)
    else:
        form = AssignmentForm()

    return render(request, 'eclass/assignment_creation.html', {'form': form, 'activity': activity})
...
def quiz_creation(request, activity_id):
    ...
    return render(request, 'eclass/quiz_creation.html', {'form': form, 'activity':activity})